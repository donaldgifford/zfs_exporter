...
2. Click **Upload JSON file** and select one of the files from `contrib/grafana/`.
...
Each dashboard uses a template variable named `datasource` of type
`prometheus`. On first load Grafana will prompt you to select a Prometheus
datasource. If you have only one Prometheus datasource it will be selected
automatically.
...
| Pool Days Until Full | `predict_linear(zfs_pool_free_bytes[7d], ...)` | Estimated days until pool fills based on 7-day trend. Red < 7d, yellow 7-30d, green > 30d. Shows "Not filling" when the pool is not growing |
...
1. Ensure the recording rules file is loaded in Prometheus. See
...
The dashboard JSON is generated by the dashgen tool. If panel layout looks
wrong after manual edits, regenerate with:
...
The default service list is NFS, SMB, and iSCSI. To change which services
