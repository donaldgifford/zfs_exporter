...
The zfs_exporter ships pre-built Prometheus recording rules and alert rules as
Kubernetes `PrometheusRule` CRs, generated into `contrib/prometheus/data/`:

| File                         | Purpose                                                                                 |
| ---------------------------- | --------------------------------------------------------------------------------------- |
| `zfs-recording-rules.yaml`  | PrometheusRule CR with pre-computed baselines for anomaly detection dashboards and alerts |
| `zfs-alerts.yaml`            | PrometheusRule CR with 21 alert rules covering exporter health, pool health, capacity, services, and anomalies |

A hand-maintained `ScrapeConfig` CR for the exporter lives in
`contrib/prometheus/scrape-configs/zfs.yaml`.
...
│   ├── zfs-alerts.yaml                # PrometheusRule CR (alert rules)
│   └── zfs-recording-rules.yaml       # PrometheusRule CR (recording rules)
├── kustomization.yaml                 # Kustomize overlay
└── scrape-configs/
    └── zfs.yaml                       # ScrapeConfig CR (hand-maintained)
```

## Installation

### Kubernetes (Kustomize + Prometheus Operator)

If you run the [Prometheus Operator](https://prometheus-operator.dev/), deploy
the rules and scrape config with Kustomize:

```bash
kubectl apply -k contrib/prometheus/
```

This creates:

- **PrometheusRule** `zfs-recording-rules` — recording rules for anomaly
  detection baselines
- **PrometheusRule** `zfs-alerts` — all 21 alert rules
- **ScrapeConfig** `zfs-server` — scrape target for the ZFS exporter

The PrometheusRule CRs are labeled `prometheus: system-rules-prometheus` and
deployed to the `monitoring` namespace. Edit the `metadata.labels` to match
your Prometheus Operator's `ruleSelector`, and update the namespace to match
your environment.

The ScrapeConfig targets `10.10.10.10:9135` as an example. Update the
`staticConfigs` target to point to your ZFS host.

### Standalone Prometheus (non-Kubernetes)

Extract the rule groups from the generated PrometheusRule CRs for use with
standalone Prometheus. The `spec.groups` field contains standard Prometheus rule
groups:

```bash
# Requires yq (https://github.com/mikefarah/yq)
yq '.spec.groups' contrib/prometheus/data/zfs-recording-rules.yaml > recording_rules.yml
yq '.spec.groups' contrib/prometheus/data/zfs-alerts.yaml > alerts.yml
```

Then add the extracted files to your `prometheus.yml`:
...
**File:** `contrib/prometheus/data/zfs-recording-rules.yaml`
...
**File:** `contrib/prometheus/data/zfs-alerts.yaml`
...
floor (1 GiB or 10% of the average, whichever is larger). This prevents false
positives on tiny datasets with naturally low variance.

## Troubleshooting

### Rules not loading

1. For Kubernetes: check that the PrometheusRule CRs are in the correct
   namespace and that their labels match your Prometheus Operator's
   `ruleSelector`.
2. For standalone: check the `rule_files` paths in `prometheus.yml` are correct
   and accessible by the Prometheus process.
3. Validate YAML syntax:

   ```bash
   # Standalone extracted rules
   promtool check rules recording_rules.yml
   promtool check rules alerts.yml
   ```

4. Check Prometheus logs for rule loading errors after reload.
5. Verify in the Prometheus UI at **Status > Rules** that both groups appear.

### Recording rule metrics missing

1. Query `zfs:dataset_used_bytes:avg7d` in Prometheus. If it returns no results:
   - The recording rules may not be loaded (see above).
...
4. **Check recording rule dependencies.** The anomaly alerts
   ...
   recording rules. If the recording rule metrics don't exist, these alerts will
   never fire.
...
After editing the generated files in `contrib/prometheus/data/`, reload
Prometheus and verify the changes in **Status > Rules**.
...
> generator source in `tools/dashgen/rules/` and regenerate.
